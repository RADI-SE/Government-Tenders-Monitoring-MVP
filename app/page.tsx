  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f6f8ff] text-slate-950">
      <nav className="relative z-10 mx-auto flex h-20 max-w-7xl items-center justify-between px-5 md:px-8">
        ...
        <div className="flex items-center gap-2"><LanguageToggle />{isLoaded && isSignedIn ? <Link href="/dashboard" className="rounded-xl bg-indigo-700 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-200 hover:bg-indigo-800">{tr("فتح لوحة التحكم", "Open dashboard")}</Link> : <><Link href="/sign-in" className="rounded-xl px-4 py-2.5 text-sm font-bold text-indigo-800 hover:bg-white">{tr("تسجيل الدخول", "Login")}</Link><Link href="/sign-up" className="rounded-xl bg-indigo-700 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-200 hover:bg-indigo-800">{tr("إنشاء حساب", "Sign up")}</Link></>}</div>
      </nav>
      ...
      <section className="relative mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl items-center gap-12 px-5 py-16 md:px-8 lg:grid-cols-[1.05fr_.95fr]">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-8 flex flex-wrap gap-3"><Link href={primaryHref} className="inline-flex items-center gap-2 rounded-xl bg-indigo-700 px-5 py-3.5 text-sm font-bold text-white shadow-xl shadow-indigo-200 hover:-translate-y-0.5">{isSignedIn ? tr("فتح لوحة التحكم", "Open dashboard") : tr("ابدأ الآن", "Get started")} <ArrowLeft className="h-4 w-4" /></Link><Link href="/dashboard" className="rounded-xl border border-indigo-100 bg-white px-5 py-3.5 text-sm font-bold text-indigo-800 shadow-sm hover:bg-indigo-50">{tr("استعراض المنصة", "Explore platform")}</Link></div>
        </div>
