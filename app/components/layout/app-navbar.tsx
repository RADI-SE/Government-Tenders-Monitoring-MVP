  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-5 backdrop-blur md:px-8">
      <strong className="text-sm text-slate-900">
        {tr("منصة متابعة المنافسات", "Government Tender Monitoring")}
      </strong>
      <div className="flex items-center gap-2">
        <LanguageToggle />
        <button
          aria-label={tr("المساعدة", "Help")}
        ...
        >
          ...
          <span className="absolute left-2 top-2 h-1.5 w-1.5 rounded-full bg-rose-500" />
        </Link>
        {isLoaded && !isSignedIn ? (
          <>
            <Link
              href="/sign-in"
              className="rounded-xl px-3 py-2 text-xs font-bold text-indigo-700 hover:bg-indigo-50"
            >
              {tr("دخول", "Login")}
            </Link>
            <Link
