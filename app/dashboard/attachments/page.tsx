...
export default function AttachmentsPage() {
  ...
  const [editing, setEditing] = useState<Doc<"attachments"> | null | undefined>(
    undefined,
  );
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const values = {
        ...
        type: String(data.get("type")) as (typeof fileTypes)[number],
      };
  ...
  async function remove(file: Doc<"attachments">) {
    if (window.confirm(tr("حذف هذا المرفق؟", "Delete this attachment?")))
  ...
  return (
    <ResourcePage
      ...
      titleEn="Documents and attachments"
      subtitle="Tender documents provided by the attachments backend."
      subtitleAr="وثائق المنافسات والمرفقات المرتبطة بها."
      icon={Paperclip}
    >
      ...
      <ResourceGrid>
        {files?.map((file) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <small className="mt-1 block uppercase text-slate-400">
                {file.type}
              </small>
            </span>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              aria-label={tr("فتح المرفق", "Open attachment")}
        ...
        {!files?.length && (
          <ResourceEmpty
            ...
            label="No attachments uploaded yet."
            labelAr="لا توجد مرفقات بعد."
          />
        )}
      </ResourceGrid>
      {editing !== undefined ? (
        <CrudDialog
          title={
            editing
              ? tr("تعديل المرفق", "Edit attachment")
              : tr("إضافة مرفق", "Add attachment")
          }
