...
export default function AnalysisPage() {
  ...
  const [editing, setEditing] = useState<Doc<"aiAnalysis"> | null | undefined>(
    undefined,
  );
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const values = {
      ...
      requiredDocuments: list(data.get("documents")),
      generatedBy: "Spiders AI MVP",
    };
    try {
  ...
  async function remove(item: Doc<"aiAnalysis">) {
    if (window.confirm(tr("حذف هذا التحليل؟", "Delete this analysis?")))
      await removeAnalysis({ id: item._id });
  }

  return (
    <ResourcePage
      eyebrow="SPIDERS INTELLIGENCE"
      title="تحليلات الذكاء الاصطناعي"
      titleEn="AI analysis"
      subtitle="Opportunity scores, recommendations, strengths, and risks."
      subtitleAr="تقييم الفرص والتوصيات ونقاط القوة والمخاطر."
      icon={BrainCircuit}
    >
      ...
      <ResourceGrid>
        {analyses?.map((item) => (
          ...
          >
            <div className="bg-gradient-to-l from-indigo-50 to-emerald-50 p-5">
              <div className="flex items-center justify-between">
                <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-bold text-indigo-700">
                  {item.classification}
                </span>
                <strong className="text-2xl text-indigo-800">
                  {item.opportunityScore}%
                </strong>
              </div>
              <h2 className="mt-4 text-sm font-bold text-slate-950">
                {tr("التوصية", "Recommendation")}: {item.recommendation}
              </h2>
            </div>
            <div className="p-5">
              <p className="text-xs leading-6 text-slate-600">{item.summary}</p>
              {item.risks.length ? (
                <div className="mt-4 flex items-start gap-2 text-xs text-rose-600">
        ...
        {!analyses?.length && (
          <ResourceEmpty
            ...
            label="No AI analyses have been generated yet."
            labelAr="لم يتم إنشاء تحليلات بعد."
          />
        )}
      </ResourceGrid>
      {editing !== undefined ? (
        <CrudDialog
          title={
            editing
              ? tr("تعديل التحليل", "Edit analysis")
              : tr("إضافة تحليل", "Add analysis")
          }
          ...
          <form onSubmit={save} className="grid gap-4 sm:grid-cols-2">
            ...
            <label className={labelClass}>
              ...
              <select
                ...
                name="classification"
                defaultValue={editing?.classification ?? "Other"}
              >
            ...
            <label className={labelClass}>
              ...
              <input
                ...
                min="0"
                max="100"
                defaultValue={editing?.opportunityScore ?? 70}
                required
              />
            </label>
            <label className={labelClass}>
              ...
              <select
                className={fieldClass}
                name="recommendation"
                defaultValue={editing?.recommendation ?? "Review"}
              >
            ...
            <ListField
              name="strengths"
              label={tr("نقاط القوة", "Strengths")}
              value={editing?.strengths}
            />
            <ListField
              name="risks"
              label={tr("المخاطر", "Risks")}
              value={editing?.risks}
            />
            <ListField
              name="documents"
              label={tr("المستندات المطلوبة", "Required documents")}
              value={editing?.requiredDocuments}
            />
