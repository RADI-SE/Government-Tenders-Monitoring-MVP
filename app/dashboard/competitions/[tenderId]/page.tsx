...
export default function TenderDetailsPage() {
  ...
  const tender = useQuery(api.tenders.getTenderById, Number.isFinite(numericId) ? { id: numericId } : "skip");
  const createTask = useMutation(api.tasks.create);

  if (tender === undefined) return <div className="p-12 text-center text-slate-500">{tr("جارٍ تحميل التفاصيل...", "Loading tender details...")}</div>;
